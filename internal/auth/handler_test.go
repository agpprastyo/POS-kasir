...
func setupHandlerTest(t *testing.T) (*mocks.MockIAuthService, *mocks.MockFieldLogger, *mocks.MockValidator, auth.IAuthHandler, *fiber.App) {
	...
	handler := auth.NewAuthHandler(mockService, mockLogger, mockValidator, cfg)
	app := fiber.New()
...
func TestAthHandler_RefreshHandler(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		mockService.EXPECT().RefreshToken(gomock.Any(), "valid_refresh").Return(&dto.LoginResponse{
			Token:        "new_access",
...
func TestAthHandler_AddUserHandler(t *testing.T) {
	...
	t.Run("UnauthorizedRole", func(t *testing.T) {
		...
		reqBodyAdmin.Role = repository.UserRoleAdmin // Higher than Cashier
...
func TestAthHandler_UpdateAvatarHandler(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		mockService.EXPECT().UploadAvatar(gomock.Any(), userID, gomock.Any()).Return(&dto.ProfileResponse{
			ID:     userID,
