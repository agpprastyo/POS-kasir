...
<div class="container mx-auto space-y-6">
	...
	<div class="flex flex-col lg:flex-row gap-6">
		...
		<aside class="w-full lg:w-72 flex-shrink-0">
			<div class="rounded-xl bg-white p-4 border border-gray-200  sticky top-6">
				<div class="flex flex-col gap-4">
					<h3 class="text-lg font-semibold text-gray-800">Filter & Tampilan</h3>

					<div class="flex rounded-lg bg-gray-100 p-1">
						<a href="?{createQueryString({ ...data.queryParams, view: 'grid' })}" class="flex-1 text-center rounded-md px-3 py-1.5 text-sm font-medium" class:bg-white={data.view === 'grid'} class:text-gray-500={data.view !== 'grid'} class:shadow={data.view === 'grid'}>Grid</a>
						<a href="?{createQueryString({ ...data.queryParams, view: 'list' })}" class="flex-1 text-center rounded-md px-3 py-1.5 text-sm font-medium" class:bg-white={data.view === 'list'} class:text-gray-500={data.view !== 'list'} class:shadow={data.view === 'list'}>List</a>
					</div>

					<form on:submit|preventDefault={handleFilterSubmit} class="flex flex-col gap-4">
						<div>
							...
							<div class="flex items-center gap-2 rounded-lg bg-gray-100 px-4 py-2">
								...
								<input type="search" id="search-filter" name="search" placeholder="Cari produk..." bind:value={search} class="w-full bg-transparent text-gray-700 placeholder-gray-400 outline-none border-0" />
							</div>
						</div>
						<div>
							<label for="category-filter" class="sr-only">Kategori</label>
							<select id="category-filter" name="category_id" bind:value={category_id} class="w-full rounded-lg border-1 border-gray-200 bg-gray-50 px-4 py-2 text-gray-700 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
						...
						<button type="submit" class="w-full rounded-lg bg-indigo-600 px-6 py-2 font-semibold text-white shadow transition hover:bg-indigo-700">Filter</button>
					</form>
		...
		<main class="flex-grow space-y-6">
			{#if data.view === 'grid'}
				<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"> {#each data.products as product (product.id)}
					<div class="group rounded-lg bg-white border border-gray-200 overflow-hidden transition-shadow hover:shadow-sm hover:border-0">
